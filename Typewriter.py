...
cap = pyscreenshot.grab(bbox=(Config["BBTopLeftX"], Config["BBTopLeftY"], Config["BBBottomRightX"], Config["BBBottomRightY"])) # Capture screen area contaning letters, may need to be customized
...
time.sleep(Config["DelayBeforeTyping"]) # Gives us a delay before the actual typing starts
...
for x in tesstr:
    ...
    ShouldMakeMistake = (Config["TargetAccuracy"] < 100) # Use TargetAccuracy to determine whether or not we should miss a letter
    if ShouldMakeMistake:
        ShouldMakeMistake = random.random() > Config["TargetAccuracy"] / 100
